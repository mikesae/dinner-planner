...
export class ItemDetail extends Component {
    ...
    async onImageChange(event: any) {
        ...
        let item: any = this.state.item;
        item.image = await storeImage(fileForUpload, this.state.userName, item.name);
        this.setState({item: item});
    }

    async updateItem() {
        await setItem(this.state.item);
    }
        ...
        return (
            <>
                {item === null ?
                    ...
                    :
                    <>
                        <TopNavbar title={item.name} showBackNav={true}
                                   backNav={item.category === 'Mains' ? '/mains' : '/sides'}/>
                        <Container>
                            <FormGroup>
                                ...
                                <Row className="px-3 pb-3">
                                    ...
                                    <input type="text" placeholder="Name" className="" value={item.name ?? ""}
                                           onChange={e => this.setItemName(e.target.value)}/>
                                </Row>
                                <Row className="px-3 pb-3">
                                    <FormLabel>Description</FormLabel>
                                    <input type="text" placeholder="Description" className=""
                                           value={item.description ?? ""}
                                           onChange={e => this.setItemDescription(e.target.value)}/>
                                </Row>
                                <Row className="px-3 pb-3">
                                    <button style={{width: "100%"}} className="btn btn-primary"
                                            onClick={() => this.updateItem()}>Update
                                    </button>
                                </Row>
                            </FormGroup>
                            <div className="spacer"/>
